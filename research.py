...
from typing import Dict, List, Tuple, Union, Optional           # Type hints for function signatures
...
def load_timeseries(
    ...
    ##So basically what we have done here is loaded the data file. now what binance gives us is a .parquet file (this is quicker to read in columns) and we have to now group these like multiple candles into bins of what ever time period we have taken in the args as time_interval. for example if we have taken 1h as the time_interval then we have to group the trades in 1 hour bins and then calculate the open, high, low, close for each bin. 
